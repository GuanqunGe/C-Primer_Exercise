...
void biggies(std::vector<std::string> &words, unsigned sz){
    ...
    //group words with length longer than sz in the first part, keep the alpha order
    auto end_partition = std::stable_partition(words.begin(), words.end(), [sz](const std::string &s) {return s.size() >= sz; });
